...
/* MODULE_SUPPORTED_DEVICE deprecated in newer kernels */
...
#ifdef MULTI_THREAD_ENCRYPT
static int ssv6xxx_cpu_callback(struct notifier_block *nfb,
                         ...
                         void *hcpu)
{
    ...
    case CPU_UP_PREPARE: {
        ...
        task_state = READ_ONCE(ta->encrypt_task->__state);
        if(task_state & TASK_UNINTERRUPTIBLE) {
            kthread_bind(ta->encrypt_task, hotcpu);
        }
        dev_dbg(sc->dev, "encrypt_task %p state is %u", ta->encrypt_task, task_state);
        break;
    }
    case CPU_ONLINE: {
        int cpu = 0;
        list_for_each_entry(ta, &sc->encrypt_task_head, list) {
            if(cpu == hotcpu) {
                ...
                if ( (ta->started == 0) && (cpu_online(cpu)) ) {
                    ...
                    ta->started = 1;
                    task_state = READ_ONCE(ta->encrypt_task->__state);
                    dev_dbg(sc->dev, "wake up encrypt_task %p state is %u, cpu = %d", ta->encrypt_task, task_state, cpu);
                }
                break;
            }
            cpu++;
...
static void ssv6xxx_preload_sw_cipher(void)
{
    ...
    /* crypto_alloc_blkcipher and crypto_alloc_cipher deprecated in kernel 5.5+ */
    /* Crypto modules are auto-loaded on demand in modern kernels */
#endif
}
static int tu_ssv6xxx_init_softc(struct ssv_softc *sc)
{
    ...
    if (SSV_NEED_SW_CIPHER(sc->sh)) {
        ...
        /* CPU hotplug notifier replaced with cpuhp state machine in kernel 4.10+ */
        /* Not implementing cpuhp for now, multi-thread encryption may not work correctly with hotplug */
    }
#endif
...
static int ssv6xxx_load_firmware(struct ssv_hw *sh)
{
    int ret=0;
...
static int tu_ssv6xxx_dev_probe(struct platform_device *pdev)
{
    struct ssv_softc *sc;
