...
static void __maybe_unused ssv6xxx_none_func(struct ssv_softc *sc)
{
...
#endif
static unsigned int skb_queue_len_safe(struct sk_buff_head *list)
{
    unsigned long flags;
    unsigned int ret = 0;
...
#ifdef MULTI_THREAD_ENCRYPT
    static bool _is_encrypt_needed(struct sk_buff *skb) {
        struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
        struct ieee80211_hdr *hdr = (struct ieee80211_hdr *) skb->data;
        struct SKB_info_st *skb_info = (struct SKB_info_st *)skb->head;
        struct ieee80211_sta *sta = skb_info->sta;
    ...
    int ssv6xxx_encrypt_task (void *data) {
        ...
        while (!kthread_should_stop()) {
            ...
            if(skb_queue_len_safe(&sc->preprocess_q) == 0 || (ta->cpu_offline != 0) ) {
                ...
                ta->running = 1;
                /* cpus_allowed renamed to cpus_ptr in kernel 5.10+ */
                CPUMask = *(cpumask_bits(current->cpus_ptr));
            }
...
*/
    ...
    static void _stop_crypto_task (struct ssv_softc *sc) {
        struct ssv_encrypt_task_list *ta = NULL;
        int num_running = 0;
        list_for_each_entry(ta, &sc->encrypt_task_head, list) {
            ta->paused = 1;
    ...
    static void _resume_crypto_task (struct ssv_softc *sc) {
        struct ssv_encrypt_task_list *ta = NULL;
        list_for_each_entry(ta, &sc->encrypt_task_head, list) {
            ta->paused = 0;
            wake_up(&ta->encrypt_wait_q);
    ...
    static void _clean_up_crypto_skb (struct ssv_softc *sc, struct ieee80211_sta *sta) {
        unsigned long flags;
        u32 sta_addr0_3 = *(u32 *)&sta->addr[0];
        u32 sta_addr4_5 = (u32)*(u16 *)&sta->addr[4];
        u32 removed_skb_num;
        ...
        if (vif->type == NL80211_IFTYPE_STATION) {
            ...
            if (changed & BSS_CHANGED_ASSOC) {
                ...
                if(!sc->isAssoc) {
                    ...
                    dev_dbg(sc->dev, KERN_INFO "!!vif->cfg.aid = %d\n", vif->cfg.aid);
...
out:
    ...
    static void ssv6200_set_default_unicast_key(struct ieee80211_hw *hw,
    ...
    void __maybe_unused ssv6xxx_power_sleep(void *param) {
    ...
    void __maybe_unused ssv6xxx_power_awake(void *param) {
        struct ssv_softc *sc = (struct ssv_softc *)param;
        SSV_PMU_AWAKE(sc);
        mutex_lock(&sc->mutex);
        ieee80211_wake_queues(sc->hw);
...
#endif
    void __maybe_unused ssv6xxx_enable_usb_acc(void *param, u8 epnum) {
        struct ssv_softc *sc = (struct ssv_softc *)param;
        SSV_ENABLE_USB_ACC(sc, epnum);
        return;
    }
    void __maybe_unused ssv6xxx_disable_usb_acc(void *param, u8 epnum) {
        struct ssv_softc *sc = (struct ssv_softc *)param;
        SSV_DISABLE_USB_ACC(sc, epnum);
        return;
    }
    void __maybe_unused ssv6xxx_jump_to_rom(void *param) {
        struct ssv_softc *sc = (struct ssv_softc *)param;
        SSV_JUMP_TO_ROM(sc);
        return;
    }
    int __maybe_unused ssv6xxx_rx_burstread_size(void *param) {
        struct ssv_softc *sc = (struct ssv_softc *)param;
        struct ssv_hw *sh = sc->sh;
        return sh->rx_burstread_size_type;
    }
    int __maybe_unused ssv6xxx_peek_next_pkt_len(struct sk_buff *skb, void *param) {
        struct ssv_softc *sc = (struct ssv_softc *)param;
        struct ssv_hw *sh = sc->sh;
    ...
    int __maybe_unused ssv6xxx_skb_pre_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc) {
...
#endif
    int __maybe_unused ssv6xxx_skb_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc) {
        struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)mpdu->data;
        int ret = 0;
#ifndef MULTI_THREAD_ENCRYPT
    ...
    struct ssv_crypto_data *ssv6xxx_skb_get_rx_cryptops(struct ssv_softc *sc, struct ieee80211_sta *sta, struct sk_buff *mpdu) {
        ...
        if((sta_priv != NULL) && (vif_info->if_type == NL80211_IFTYPE_AP)) {
            up_read(&sc->sta_info_sem);
            return &sta_priv->crypto_data;
        } else if((sta_priv != NULL) && (unicast == 1)) {
            up_read(&sc->sta_info_sem);
            return &sta_priv->crypto_data;
...
#endif
    int __maybe_unused ssv6200_tx_flow_control(void *dev, int hw_txqid, bool fc_en,int debug) {
    ...
    void __maybe_unused ssv6xxx_tx_q_empty_cb (u32 txq_no, void *cb_data) {
...
#define MAX_CCI_LEVEL 128
    ...
    static u64 ssv6200_get_systime_us(struct ssv_softc *sc)
    {
    ...
    static void _proc_data_rx_skb (struct ssv_softc *sc, struct sk_buff *rx_skb) {
...
#endif
            mgmt_tmp->u.beacon.timestamp = cpu_to_le64(ssv6200_get_systime_us(sc)+ 10*1000*1000);
        ...
        if(ieee80211_is_probe_resp(hdr->frame_control)) {
            ...
            mgmt_tmp = (struct ieee80211_mgmt *)(rx_skb->data + SSV6XXX_RX_DESC_LEN );
#endif
            mgmt_tmp->u.probe_resp.timestamp = cpu_to_le64(ssv6200_get_systime_us(sc)+ 10*1000*1000) ;
...
#endif
    static int _process_rx_umac(struct ssv6xxx_umac_ops *umac, struct sk_buff *skb) {
...
#define BEACON_MISS_RSSI_THRESHOLD (-80)
    void __maybe_unused ssv6xxx_beacon_miss_work(struct work_struct *work) {
...
out:
    ...
    void __maybe_unused _process_rx_q (struct ssv_softc *sc, struct sk_buff_head *rx_q, spinlock_t *rx_q_lock) {
    ...
    int __maybe_unused ssv6200_is_rx_q_full(void *args) {
        struct ssv_softc *sc=args;
...
#else
    int __maybe_unused ssv6200_rx(struct sk_buff *rx_skb, void *args)
#endif
    {
        struct ssv_softc *sc=args;
#ifdef IRQ_PROC_RX_DATA
    ...
    void __maybe_unused ssv6xxx_rx_stuck_process(struct work_struct *work) {
    ...
    void __maybe_unused ssv6xxx_mib_edca_process(struct work_struct *work) {
        struct ssv_softc *sc = container_of(work, struct ssv_softc, mib_edca_work);
        if (!sc->sh)
            return;
        SSV_EDCA_STAT(sc->sh);
    }
    void __maybe_unused ssv6xxx_tx_poll_process(struct work_struct *work) {
...
#ifdef CONFIG_SSV_CCI_IMPROVEMENT
    void __maybe_unused ssv6xxx_cci_clean_process(struct work_struct *work) {
    ...
    void __maybe_unused ssv6xxx_cci_set_process(struct work_struct *work) {
...
#endif
    void __maybe_unused ssv6xxx_set_txpwr_process(struct work_struct *work) {
    ...
    void __maybe_unused ssv6xxx_thermal_monitor_process(struct work_struct *work) {
...
#endif
    struct ieee80211_sta * __maybe_unused ssv6xxx_find_sta_by_rx_skb (struct ssv_softc *sc, struct sk_buff *skb) {
    ...
    struct ieee80211_sta * __maybe_unused ssv6xxx_find_sta_by_addr (struct ssv_softc *sc, u8 addr[6]) {
        struct ieee80211_sta *sta;
        int i;
        for (i = 0; i < SSV6200_MAX_VIF; i++) {
            if (sc->vif_info[i].vif == NULL)
    ...
    void __maybe_unused ssv6xxx_foreach_sta (struct ssv_softc *sc, void (*sta_func)(struct ssv_softc *, struct ssv_sta_info *, void *), void *param) {
        int i;
    ...
    void __maybe_unused ssv6xxx_foreach_vif_sta (struct ssv_softc *sc,
                                  struct ssv_vif_info *vif_info,
